...
// function renderGrid(grid: GridCell[][]): void {
//   for (let i = 0; i < grid.length; i++) {
//     let line = "";
//     for (let j = 0; j < grid[i].length; j++) {
//       line += (grid[i][j] ? chalk.green("#") : chalk.red("."));
//     }
//     console.log(console);
//   }
...
function mainCalculations(dice: Dice, minimumScore: number) {
  const data = loadData();
  ...
  for (let round = 0; round < minimumScore * 2; round++) {
    ...
    console.log(
      chalk.blue`Player ${playerIndex + 1} rolled ${diceRolls.join(",")} (${diceRollTotal}), scored ${newPosition} (total ${
...
export function Part1() {
  ...
  const players = mainCalculations(dice, 1000);

  console.log(players);

  const sortedPlayers = [...players].sort((a, b) => b.score - a.score);

  console.log(chalk.red.bold`Answer: ${sortedPlayers[1].score * dice.rollCount}`);
}
...
export function Part2() {
  ...
  for (let round = 0; round < 42; round++) {
    console.log(chalk.gray`Round ${round} - ${[...player.positionScoreCountMap.values()].reduce((a,b) => a + b).toString()} Total Games in Progress`);
    const playerIndex = round % 2;

    console.log("S", player.positionScoreCountMap.size);
    ...
    for (const [key, count] of player.positionScoreCountMap.entries()) {
      ...
      for (let i = 3; i <= 9; i++) {
        ...
        const key: `${number}-${number}-${number}-${number}` = playerIndex === 0 ? `${newPosition}-${positionB}-${newScore}-${scoreB}` : `${positionA}-${newPosition}-${scoreA}-${newScore}`;
        if (newMap.has(key)) {
