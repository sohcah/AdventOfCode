...
for (const line of input[1]) {
    ...
    for (let i = 0;i<Number(trio[0]);i++) {
        stacks[toStack] += stacks[fromStack][stacks[fromStack].length-1];
        stacks[fromStack] = stacks[fromStack].slice(0,stacks[fromStack].length-1);
    }
}
