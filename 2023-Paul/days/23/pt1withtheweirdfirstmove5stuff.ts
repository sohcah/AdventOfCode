...
let nodes = [];
let twoWayTable = Array(15).fill().map(() => Array(15).fill(0)); // distance
function nextNode(d,a, direction) {
	...
	let firstMove = true;
	let oneWay = false;
	let surround = [3,0,4,3]; //up,right,down,left (although this one is just a random starter)
	while (((lastD===19&&lastA===13,true)?console.log(d,a,surround,surround.filter(i=>i>2&&(firstMove||i!=5)).length):0),surround.filter(i=>i>2&&(firstMove||i!=5)).length == 3) {
		...
		((lastD===19&&lastA===13,true)?console.log(d,a,surround,surround.filter(i=>i>2&&(firstMove||i!=5)).length):0)
		if (surround.filter(i=>i>2&&(firstMove||i!=5)).length >= 3) {
			input[d][a] = 4;
