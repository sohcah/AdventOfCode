...
export function Part2() {
  ...
  let pairs: Record<string, {letter: string, array: number[]}> = Object.fromEntries(lines[1].split("\n").map(i => i.split(" -> ") as [string, string]).map(i => {
    const arr = new Array(26).fill(0);
    arr[i[1].toString().charCodeAt(0) - 65] = 1;
    return [i[0], {letter: i[1], array: arr}] as [string, {letter: string, array: number[]}];
  }));
  ...
  for (let i=1;i<40;i++) {
    ...
    for(const pair in pairs) {
      for (let j=0;j<26;j++) {
        nextPairs[pair].array[j]=pairs[pair[0]+pairs[pair].letter].array[j]+pairs[pairs[pair].letter+pair[1]].array[j];
