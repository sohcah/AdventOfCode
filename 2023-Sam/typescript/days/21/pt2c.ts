...
const xWidth = width * 11;
function getCoord(x: number, y: number) {
  return x + xOffset + (y + yOffset) * xWidth;
}
const max = xWidth * height * 11;

if (width !== height) throw new Error("Width and Height must be equal");

const oddPositions = new ArraySet(max);
const evenPositions = new ArraySet(max);
...
for (let step = 0; step < N * 6 + 2; step++) {
  ...
  if (step === N * 2 || step === N * 2 - 1) {
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < height; x++) {
        if (newPositionsSet.has(getCoord(x, y))) centerCount++;
      }
    }
    if (step % 2 === 0) {
    ...
    if (step % height === (STEPS % height) - 1) {
      for (let y = 0; y < height; y++) {
        for (let x = width; x < width * 2; x++) {
          if (newPositionsSet.has(getCoord(x, y))) edgeCount++;
        }
      }
  ...
  if (step >= N * 2 && step < N * 6 + 2) {
    ...
    if (step % height === (STEPS % height) - 1) {
      ...
      for (let y = height; y < height * 2; y++) {
        for (let x = width; x < width * 2; x++) {
          if (newPositionsSet.has(getCoord(x, y))) cornerCount++;
        }
      }
      // bottom left
      for (let y = height; y < height * 2; y++) {
