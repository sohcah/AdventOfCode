...
while (!positions.isEmpty()) {
  ...
  if (x < width - 1 && (rotation !== 0 || noSteps < 3) && rotation !== 2) {
    positions.push([x + 1, y, 0, rotation === 0 ? noSteps + 1 : 1, heatLoss + data[x + 1 + y * width]]);
  }
  if (x > 0 && (rotation !== 2 || noSteps < 3) && rotation !== 0) {
    positions.push([x - 1, y, 2, rotation === 2 ? noSteps + 1 : 1, heatLoss + data[x - 1 + y * width]]);
  }
  if (y < height - 1 && (rotation !== 1 || noSteps < 3) && rotation !== 3) {
    positions.push([x, y + 1, 1, rotation === 1 ? noSteps + 1 : 1, heatLoss + data[x + (y + 1) * width]]);
  }
  if (y > 0 && (rotation !== 3 || noSteps < 3) && rotation !== 1) {
    positions.push([x, y - 1, 3, rotation === 3 ? noSteps + 1 : 1, heatLoss + data[x + (y - 1) * width]]);
  }
}
console.log(n);
