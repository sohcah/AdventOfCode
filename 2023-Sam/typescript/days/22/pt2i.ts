...
let sum = 0;
const eliminatedBricks = new ArraySet(bricks.length);
for (const brick of bricks) {
  eliminatedBricks.clear();
  eliminatedBricks.add(brick.index);
  ...
  for (let otherIndex = brick.index + 1; otherIndex < bricks.length; otherIndex++) {
    ...
    if (iteratorEvery(other.dependentOn, (i) => eliminatedBricks.has(i.index))) {
      eliminatedBricks.add(otherIndex);
      lastValidLayer = Math.max(lastValidLayer, other.maxZ + 1);
    }
  }
  sum += eliminatedBricks.size - 1;
}
