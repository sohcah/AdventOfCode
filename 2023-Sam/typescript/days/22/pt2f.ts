...
const nonDisintegratableBricks = input.filter((brick) => {
...
function collapseFake(
  internalInput: Brick[],
  internalBricksAtCoords: Uint16Array,
  initialCollapsed: number
) {
  collapsed.add(initialCollapsed);
  for (const brick of internalInput) {
    if (brick.id === initialCollapsed) continue;
    const canMoveDown = brick.rangeForBelow.every((i) => {
      if (i < zMult) return false;
      const b = internalBricksAtCoords[i - zMult];
      return b === brick.id || b === NULL || collapsed.has(b);
    });
    if (!canMoveDown) continue;
    collapsed.add(brick.id);
  }
  const size = collapsed.size;
  collapsed.clear();
  return size;
}

input.forEach((i) => {
  if (i.directionIndex === 2) {
    i.rangeForBelow = [getCoord(...i.from)];
  } else {
    i.rangeForBelow = getCoordRange(i);
  }
});

let sum = 0;
for (const brick of nonDisintegratableBricks) {
  const collapsed = collapseFake(input, bricksAtCoords, brick.id) - 1;
  sum += collapsed;
}
