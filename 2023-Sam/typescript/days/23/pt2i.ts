...
const mappedGraph = Object.fromEntries(
  Object.entries(graph).map(([key, value]) => {
    return [nodeMapping[key], value.map((i) => [nodeMapping[i[0]], i[1]] as [number, number])];
  })
);

console.log(Object.keys(nodeMapping).length);

{
  let p = 0;
  const positions = new Array<[number[], number, number]>();
  const mappedEnd = nodeMapping[endPos];
  // const startArray = 2 << nodeMapping[startPos];
  positions.push([[nodeMapping[startPos]], 0, nodeMapping[startPos]]);
