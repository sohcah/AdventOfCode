...
const triedJumpTo = new ArraySet(width * height);
...
while (positions.length) {
  ...
  const jtp = jumpTo[pos];
  if (jtp !== null) {
    const arr = [...pathSet, ...jtp!.slice(0, -1)];
    const set = new Set(arr);
    if (arr.length === set.size) {
      addPath([set, jtp!.at(-1)!], true);
      continue;
    }
  }
  ...
  if (data[pos] !== ".") {
    ...
    if (!triedJumpTo.has(pos)) {
      triedJumpTo.add(pos);
      // console.log("trying jump to", pos);
      let i = 0;
  ...
  if (pos === endPos) {
    ...
    if (endPathLength > bestEndPath) bestEndPath = endPathLength;
