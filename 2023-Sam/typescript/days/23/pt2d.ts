...
const positions = new FlatQueue<[Set<number>, number]>();
positions.push([new Set([startPos]), startPos], 1);

function addPath(path: [Set<number>, number], end = false) {
  if (path[0].has(path[1])) return;
  const newSet = new Set(path[0]);
  newSet.add(path[1]);
  positions.push([newSet, path[1]], width * height - newSet.size);
}
...
const noJump = new Uint8Array(width * height).fill(0);
...
while (positions.length) {
  ...
  if (jumpTo[pos] !== null) {
    ...
    if (noJump[pos]) {
      console.log('no jump', pos, data[pos]);
    } else {
      // noJump[pos] = 1;
    }
  }
      ...
      for (const p of [...pathSet].reverse().slice(1)) {
        ...
        if (data[p] !== ".") {
          if (i > 4) {
            ...
            triedJumpToLookup[pos] = 200;
          }
          triedJumpToLookup[pos]++;
