...
export type ResultOf<T extends ImplicitParser> = GetParserResult<
  ImplicitParserType<T>[ParseFnSymbol]
>;
...
export interface Parser<TResult> {
  ...
  dict: TResult extends DictEntry<infer TKey, infer TValue>[]
    ? (defaultValues?: Record<TKey, TValue>) => UnnamedParser<Record<TKey, TValue>>
    : never;
...
function regexp<TResult = string>(
  regexp: RegExp,
  parse: (text: string, value: RegExpMatchArray) => TResult = (i) => i as TResult
) {
  ...
  return custom(
    (text) => {
      ...
      if (!result)
        return fail<TResult>(
          `Unable to match ${regexp.toString()} with '${text}'`,
...
function text<TResult = string>(textMatch: string, result: TResult = text as TResult) {
  return custom((text) => {
    if (!text.startsWith(textMatch))
      return fail<TResult>(
        `Unable to match ${textMatch} with '${text}'`,
...
type ImplicitParserType<TParser extends ImplicitParser> = TParser extends UnnamedParser<any>
  ? TParser
  : TParser extends NamedParser<any, any>
    ? TParser
    : TParser extends RegExp
      ? UnnamedParser<string>
...
type FilterTuple<T extends readonly any[], E> = T extends readonly [infer F, ...infer R]
  ? readonly [F] extends readonly [E]
    ? readonly [F, ...FilterTuple<R, E>]
    : FilterTuple<R, E>
  : [];

type SeqObjNamedParsers<TParsers extends readonly ImplicitParser[]> = FilterTuple<
  TParsers,
  {
...
type NumberFrom<T extends string | number> = T extends `${infer N extends number}`
  ? N
  : T extends number
    ? T
    : never;
...
type Max<NN extends number, _a extends 0[] = []> = number extends NN
  ? number
  : _a["length"] extends 999 | NN
    ? _a["length"] extends 999
      ? number
...
type SeqObjObjectOutput<TParsers extends readonly NamedParser<any, any>[]> = UnionToIntersection<
  {
    [key in keyof TParsers]: {
      readonly [key2 in TParsers[key]["nameValue"]]: ResultOf<TParsers[key]>;
    };
  }[number]
>;

type SeqObjTupleOutput<TParsers extends readonly NamedParser<number, any>[]> = Omit<
  Tuple<ResultOf<TParsers[number]>, TSNumber.Add<1, Max<TParsers[number]["nameValue"]>>>,
  number
> &
...
function seq<const TParsers extends ImplicitParser[]>(
  strings: TemplateStringsArray,
  ...implicitParsers: ValidateSeqInput<TParsers>
): UnnamedParser<SeqObjOutput<TParsers>> {
  let parsers = implicitParsers.map(implicit);
  const shouldReturnObject = parsers.some(
    (i) => "nameValue" in i && typeof i.nameValue === "string"
  );
  let namedParsers = parsers.filter((i) => "nameValue" in i) as NamedParser<any, any>[];
  if (namedParsers.length === 0) {
    if (
      process.env.EXPERIMENTAL_OPTIMISER === "true" &&
      parsers.every((i) => i.optimiserHints.regexp)
    ) {
          ...
          strings
            .map(
              (i, n) =>
                `${escapeRegExp(i)}${
                  parsers[n]
                    ? `(?<optimizer_${optimizerId}_${n}>${
                        parsers[n].optimiserHints.regexp?.source ?? ""
                      })`
                    : ""
                }`
            )
  ...
  return custom((text) => {
    ...
    const result = (
      shouldReturnObject
        ? {}
        : new Array((namedParsers.map((i) => i.nameValue).max() as number) + 1).fill(null!)
    ) as SeqObjOutput<TParsers>;
    for (let i = 0; i < strings.length; i++) {
      if (!text.slice(taken).startsWith(strings[i])) {
        return fail<SeqObjOutput<TParsers>>(
          `Unable to match '${strings[i]}' with '${text.slice(
            taken
          )}' in '${text}' at index ${taken}`,
          `seq(${strings.raw.join("{}").replace("\n", "\\n")}`
        );
      }
      ...
      if (parserResult.type === "fail")
        return extendFail<SeqObjOutput<TParsers>>(parserResult, `seq(${strings.raw.join("{}")}`);
    ...
    return success(
      taken,
      namedParsers.length === 1 ? (Object.values(result as any)[0] as any) : result
    );
  });
}

function sep<const TParser extends ImplicitParser>(
  parser: TParser,
  sep: ImplicitParser
): Parser<ResultOf<TParser>[]> {
  ...
  return custom((text) => {
    ...
    while (finalResult.taken < text.length) {
      if (loops++ > 1000)
        return fail<ResultOf<TParser>[]>(
          "Over 1000 loops",
          `sep(${parser.toString()}, ${sep.toString()})`
        );
      const result = mainParser[parseFnSymbol](
        text.slice(finalResult.taken + previousSeparatorTaken)
      );
      if (result.type === "fail") {
        if (finalResult.taken === 0) {
          return extendFail<ResultOf<TParser>[]>(
            result,
            `sep(${parser.toString()}, ${sep.toString()})`
          );
        }
...
function or<TParser extends ImplicitParser>(
  ...implicitParsers: TParser[]
): UnnamedParser<ResultOf<TParser>> {
  ...
  return custom((text) => {
    ...
    return fail<ResultOf<TParser>>(
      `Unable to match any of ${parsers.map((i) => i.toString()).join(", ")} with '${text}'`,
      `or(${parsers.map((i) => i.toString()).join(", ")})`
    );
  });
}

function parseText<TParser extends ImplicitParser>(
  parserBuilder: TParser,
  text: string
): ResultOf<TParser> {
  ...
  if (result.taken < text.length) {
    console.warn(
      `No parsers remaining with '${text.slice(result.taken)}' in '${text}' at index ${
        result.taken
      }`
    );
  }
...
function dict<TKey extends keyof any, TValue>(
  entries: DictEntry<TKey, TValue>[]
): { [key in TKey]: TValue } {
...
function dictWithDefault<TKey extends keyof any, TValue>(defaultValue: {
  [key in TKey]: TValue;
}): typeof dict {
...
export function p<const TParsers extends ImplicitParser[]>(
  strings: TemplateStringsArray,
  ...parsers: ValidateSeqInput<TParsers>
): UnnamedParser<SeqObjOutput<TParsers>>;
export function p<TParser extends ImplicitParser>(parser: TParser): ImplicitParserType<TParser>;
export function p<TParsers extends ImplicitParser[]>(
  parsers: TParsers
): UnnamedParser<ResultOf<TParsers[number]>>;
export function p(
  ...[mainParam, ...params]:
