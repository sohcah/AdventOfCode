...
export interface Parser<TResult> {
  ...
  list<TSeparatorParser extends ImplicitParser>(
    separatorParser: TSeparatorParser
...
export type ParserOf<TParser extends ImplicitParser> = TParser extends NamedParser<any, any>
  ? TParser
  : TParser extends UnnamedParser<any>
    ? TParser
...
function dict<const TKey extends keyof any, const TValue>(
...
function dictWithDefault<const TKey extends keyof any, const TValue>(defaultValue: {
  [key in TKey]: TValue;
}): typeof dict {
  return (entries) => ({ ...defaultValue, ...dict(entries) });
}

function createParser<const TResult>(
  contentBuilder: (
    result: string,
    taken: string,
    breakLabel: string
  ...
  toNamed.generateCode = ({ useTextAfter = false }: GenerateCodeOptions = {}) => {
    const id = nanoid();
  ...
  toNamed.list = <const TSeparatorParser extends ImplicitParser>(
    separatorParser: TSeparatorParser
  ): any => {
  ...
  toNamed.map = <const TNewResult>(fn: (result: TResult) => TNewResult): any => {
    ...
    // @ts-expect-error Adding arbitrary properties to globalThis
    globalThis[id] = fn;
...
export const regexp = (regexp: RegExp) =>
  createParser<string>(function* (result, taken) {
...
export const text = (text: string) =>
  createParser<string>(function* (result, taken) {
    yield `if (text.slice(0,${text.length}) !== ${JSON.stringify(
      text
    )}) ${result}="Unable to match text (\\"${JSON.stringify(text).slice(
      1,
      -1
    )}\\") with " + text,${taken}=FAIL;
else ${result}=${JSON.stringify(text)},${taken}=${text.length};`;
  });

export const or = <const TParsers extends ImplicitParser[]>(...parsers: TParsers) =>
  createParser<ResultOf<TParsers[number]>>(function* (result, taken, breakLabel) {
...
export const sequence = <const TParsers extends ImplicitParser[]>(...parsers: TParsers) =>
  createParser<{
...
export const list = <
  const TItemParser extends ImplicitParser,
  TSeparatorParser extends ImplicitParser,
>(
  itemParser: TItemParser,
  separatorParser: TSeparatorParser
) =>
  createParser<ResultOf<TItemParser>[]>(function* (result, taken, breakLabel) {
...
type SeqObjHasOnlyOneNamedParser<TParsers extends readonly ImplicitParser[]> = SeqObjNamedParsers<
  GetNamedParsersConverting<TParsers>
>["length"] extends 1
  ? true
  : false;
...
type SeqObjObjectOutput<TParsers extends readonly NamedParser<any, any>[]> = UnionToIntersection<
  {
    [key in keyof TParsers]: {
      readonly [key2 in TParsers[key][InternalNameSymbol]]: ResultOf<TParsers[key]>;
    };
  }[number]
>;

type SeqObjTupleOutput<TParsers extends readonly NamedParser<any, number>[]> = Omit<
  Tuple<ResultOf<TParsers[number]>, TSNumber.Add<1, Max<TParsers[number][InternalNameSymbol]>>>,
  number
> &
...
export const seq = <const TParsers extends ImplicitParser[]>(
  strings: TemplateStringsArray,
  ...implicitParsers: TParsers
): UnnamedParser<SeqObjOutput<TParsers>> => {
  let parsers = implicitParsers.map(fromImplicit);
  let namedParsers = parsers.filter((i) => i[internalNameSymbol] !== null) as NamedParser<
    any,
    any
  >[];
  ...
  const baseSequence = sequence(
    ...strings.flatMap((i, index) => [text(i), parsers[index] || text("")])
  );
...
export function p<const TParsers extends readonly ImplicitParser[]>(
  strings: TemplateStringsArray,
  ...parsers: TParsers
): UnnamedParser<SeqObjOutput<TParsers>>;
