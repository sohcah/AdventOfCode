...
function lookupRev(key: Exclude<keyof typeof input, "seedPairs">, destination: number) {
  const data = input[key];
  const result = data.find(i => i.destination[0] <= destination && i.destination[1] >= destination);
...
const intersectionPoints = ([
  ...
  "humidityToLocation"
] as Exclude<keyof typeof input, "seedPairs">[]).reverse().reduce((a, b) => {
  const data = input[b];
  // console.log(data);
  return [
    ...a.map(dest => {
...
const results = seeds.map((seed) => {
  return ([
    ...
    "temperatureToHumidity",
    "humidityToLocation"
  ] as Exclude<keyof typeof input, "seedPairs">[]).reduce((a, b) => lookup(b, a), seed);
});
