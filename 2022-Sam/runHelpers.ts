...
async function callDay(day: string, part: string, input: DayInput, log = true): Promise<DayResult> {
    ...
    const proc = child_process.spawn(
        // `yarn workspace ${day} tsup ${path} --format esm --out-dir ${tmpDir} --minify && node ${tmpDir}/pt${part}.mjs`,
        useNode ? `yarn workspace ${day} tsx ${path}` : `bun run ${path}`,
        {
            cwd: dir,
            shell: true,
            env: {
...
export async function runDay(day: string, part: string, log = true): Promise<DayResult & { type: "result"; outerTime: number; stabilisedAt?: number }> {
