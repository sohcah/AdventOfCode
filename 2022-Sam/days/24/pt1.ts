...
for (let round = 0; round < 2000; round++) {
	...
	blizzards = blizzards.map((blizzard) => {
		return [
			(blizzard[0] + blizzardDirs[blizzard[2]][0]) % width,
			(blizzard[1] + blizzardDirs[blizzard[2]][1]) % height,
			blizzard[2],
		];
	});
	...
	paths = paths.flatMap((path) => {
		...
		for (const dir of dirs) {
			...
			if (
				(newLoc[1] < 0 || newLoc[1] >= height) &&
				newLoc.join("|") !== startPos.join("|") &&
				newLoc.join("|") !== endPos.join("|")
			)
				continue;
