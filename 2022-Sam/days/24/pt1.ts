import {CountedSet, IS_TEST, loadLines, output, SSet} from "aocutils";
...
let answer = -1;
  ...
  blizzards.sort((a,b)=>a[0]-b[0]).sort((a,b)=>a[1]-b[1]);
  ...
  if(paths.some(i => i.at(-1).join("|") === endPos.join("|"))) {
    console.log(paths.find(i => i.at(-1).join("|") === endPos.join("|")))
    console.log("Found path");
    answer = round + 1;
    break;
  }

  paths = paths.groupBy(i => i.at(-1).join("|")).valuesArray().map(i => i[0]);
