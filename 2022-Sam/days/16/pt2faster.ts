...
const N = stabilise(1000, 200, 6);

const valves = loadLines()
	.map((i) => i.split("; "))
	.map((i) => ({
		id: i[0].slice(6, 8),
...
for (let move = 1; move <= 52; move++) {
		...
		if (move % 2 === 1) {
			...
			if (valve.flow > 0 && !path[2].has(path[0])) {
				newPaths.push([
					path[0],
					path[1],
					new Set([...Array.from(path[2]), path[0]]),
					path[3] + path[4],
					path[4] + valve.flow,
				]);
			}
			...
			for (const tunnel of valve.tunnels) {
				newPaths.push([path[0], tunnel, path[2], path[3], path[4]]);
			}
			if (valve.flow > 0 && !path[2].has(path[1])) {
				newPaths.push([
					path[0],
					path[1],
					new Set([...Array.from(path[2]), path[1]]),
					path[3],
					path[4] + valve.flow,
				]);
			}
	...
	if (move % 2 === 1) {
		const grouped = newPaths.groupBy((i) =>
			[i[0], i[1], Array.from(i[2]).sort().join(",")].join(",")
		);
		paths = [];
...
output(best[3]).forTest(1707).forActual(2425);
