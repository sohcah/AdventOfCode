...
const stacks = Object.fromEntries(new Array(stackInput.split("\n").at(-1).trim().split("   ").length).fill(0).map((_, n) => {
  return stackInput.split("\n").map(i => i.slice(n * 4 + 1, n * 4 + 2)).filter(i => i !== "" && i !== " ").reverse();
}).map((i, n) => [n + 1, i]));
...
const answer = Object.values(stacks).map(i => i.at(-1) || " ").join("");

output(answer).forTest("CMZ");
